...
/// メンバを全て clear する。( ※ 試験実装 )
void clearAll( T )( ref T obj )
{
	static if     ( ( is( T U : U[] ) && ( is( U == class ) || is( U == struct ) ) )
	             || ( is( T V : V[W], W ) && ( is( V == class ) || is( U == struct ) ) ) )
	{
		foreach( one ; obj ) clearAll( one );
	}
	else static if( !is( T == class ) && !is( T == struct ) ) return;
	else foreach( one ; __traits( allMembers, T ) )
	{
		static if( __traits( compiles, __traits( getMember, obj, one ).offsetof ) )
			clearAll( __traits( getMember, obj, one ) );
	}

	static if     ( is( T == class ) && __traits( hasMember, obj, "clear" ) )
	{
		if( null !is obj ) { obj.clear(); obj = null; }
	}
	else static if( is( T == struct ) && __traits( hasMember, obj, "clear" ) ) obj.clear();
}
...
{ mixin SDLIdleWindowMix!() SWM;
	...
	this()
	{
		...
		dsan = new MikotoActor( &toUTF8, "dsan\\DさんMove.mks", "dsan\\normal1.mkm" );
		...
		dsan_drawer = new DsanDrawer( dsan.bsys, projlook, &toUTF8 );
...
class DsanDrawer
{
	...
	this( BoneSystem bsys, ref Matrix4f projlook, string delegate(jstring) toUTF8 )
	{
		assert( 0 < bsys.bones.length );
		...
		vao = new VertexArrayObject[ bsys.skins.length ];
		foreach( i, ref one ; vao ) one = new VertexArrayObject( prog, bsys.skins[i] );

		foreach( bone ; bsys.bones ) // ボーン
		{
			foreach( part ; bone.parts ) // レイヤー( == ObjectChunk )
			{
				foreach( face ; part.faces ) // マテリアル
				{
					auto d = vao[ part.vertex_id ].getDrawer!GL_TRIANGLES( face.faces );
					...
					_draws ~= ( color, use_texture, draw, b0, b1, b2 )
					{
						return
						{
							...
							if( null !is b1 )
							{
								prog[ "transform_id", 1 ] = b1.id;
								prog[ "transform", 1 ] = b1.transform;
							}
							else prog[ "transform_id", 1 ] = 4;
							if( null !is b2 )
							{
								prog[ "transform_id", 2 ] = b2.id;
								prog[ "transform", 2 ] = b2.transform;
							}
							else prog[ "transform_id", 2 ] = 4;
					...
					}( face.material.color, use_tex, d, bone, bone.parent, bone.big_brother );
				}
			}
		}
